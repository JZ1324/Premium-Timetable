...
function fixEnglishTruncation(jsonContent, approxErrorPosition = 0) {
  ...
  // If we reached here, the JSON is severely truncated
  // Try to extract at least the days and periods sections
  const daysSection = jsonContent.match(/\"days\"\s*:\s*\[[\s\S]*?\]/);
  const periodsSection = jsonContent.match(/\"periods\"\s*:\s*\[[\s\S]*?\]/);
  
  if (daysSection && periodsSection) {
    const minimalJson = `{
      ${daysSection[0]},
      ${periodsSection[0]},
      "classes": {}
    ...
    try {
      // Try to parse the minimal JSON to validate it
      JSON.parse(minimalJson);
      console.log("Created minimal valid JSON with days and periods");
      return minimalJson;
    } catch (e) {
      console.error("Failed to create minimal valid JSON:", e);
    }
  }
  
  // As a last resort, just return the original JSON
  // The calling code will need to handle the parse error
  return jsonContent;
}

/**
 * More complete recovery for English truncation issues with periods, days, and classes
 * @param {string} jsonContent - The truncated JSON to fix
 * @returns {Object|null} - Parsed JSON object or null if recovery failed
 */
function recoverFromEnglishTruncation(jsonContent) {
  try {
