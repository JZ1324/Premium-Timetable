// Import the class data enhancer
...
const parseJsonTimetable = (jsonText, setIsProcessing, setParseError) => {
    try {
            ...
            if (!jsonString) {
                ...
                try {
                    ...
                    if (repairedJson) {
                        ...
                        // Enhance the repaired JSON with structured class objects
                        const enhancedData = enhanceTimetableData(parsedJson);
                        console.log("Returning enhanced data from repaired JSON");
                        return enhancedData;
                    }
                } catch (repairError) {
        ...
        console.log("JSON validation passed, enhancing class data...");
        // Enhance the parsed JSON with structured class objects
        const enhancedData = enhanceTimetableData(parsedJson);
        console.log("Returning enhanced data with structured class objects");
        setIsProcessing(false);
        return enhancedData;
    } catch (error) {
